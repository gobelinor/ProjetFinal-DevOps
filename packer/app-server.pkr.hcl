source "openstack" "app-server" {
  ...
  image_name        = "app-server-${formatdate("YYYYMMDD-hhmm", timestamp())}"
  ...
  metadata = {
    os_type     = "linux"
    image_type  = "app-server"
    created_by  = "packer"
  }
}

build {
  sources = ["source.openstack.app-server"]
